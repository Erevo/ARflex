...
public class TurretScr : MonoBehaviour
{
    ...
    bool Search() // разворот башни на цель
    {
        ...
        Vector3 lookPos = (target.position + new Vector3(0, target.lossyScale.y / 2, 0)) - turretRotation.position;
    ...
    void Shot()
    {
        ...
        if (curFireRate > fireRate)
        {
            ...
            {
                TurretBullet bullet = Instantiate(bulletPrefab, point.position, point.rotation) as TurretBullet;
    ...
    void LateUpdate()
    {
        if (target != null && target.gameObject.GetComponent<ZombieScr>().heal > 0)
        {
